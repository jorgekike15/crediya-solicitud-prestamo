...
class SolicitudUseCaseTest {
    ...
    void findSolicitudPendienteRechazadaRevision_retornaSolicitudesFiltradas() {
        ...
        when(solicitudRepository.countByIdEstadoIn(any())).thenReturn(Mono.just(2L));

        StepVerifier.create(solicitudUseCase.findSolicitudPendienteRechazadaRevision(0, 10))
                .assertNext(response -> {
                    assert response.getSolicitudes().size() == 2;
                    assert response.getSolicitudes().get(0).getTasaInteres() == 5.0;
                    assert response.getSolicitudes().get(1).getTasaInteres() == 7.5;
                })
                .verifyComplete();
